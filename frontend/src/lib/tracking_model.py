...
def create_text_random_voice(language: str):
    text = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": f"Bạn là một trợ lý AI. Tạo cho tôi một đoạn văn thực hiện dự án voice cloning với ngôn ngữ là {language}"},
            {"role": "user", "content": f"Tạo cho tôi đoạn văn ngắn gọn khoảng 20 đến 30 từ với ước tính thời gian để tôi voice cloning khoảng 10 giây. Chỉ đưa ra đoạn text thôi không đưa ra gì thêm. Giọng văn phải tự nhiên, không máy móc và đoạn văn đó không phải là mời mở đầu có thể là một đoạn văn ngẫu nhiên nào đó khiến cho người dùng khi nhìn vào là muốn đọc  và đoạn văn phải có ý nghĩa. Ngôn ngữ: {language}"}
        ]
    )
